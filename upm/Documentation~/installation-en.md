...
```url
https://openupm.com/packages/com.moroshka.type-analyzer.html
```
...
``` bash
openupm add com.moroshka.type-analyzer
```
...
```url
https://github.com/moroshka-game/moroshka-type-analyzer.git?path=upm#v#.#.#
```
...
``` url
https://github.com/moroshka-game/moroshka-type-analyzer/releases/
```

## NuGet

```url
https://www.nuget.org/packages/Moroshka.Typeanalyzer
```
...
3. Open the manager, search for the package by name `Moroshka.TypeAnalyzer`.
...
```sh
dotnet add package Moroshka.TypeAnalyzer -v #.#.#
```
